...
IExchange {
    ...
    mapping(uint256 => address) public orderRecipient;
    ...
    */
    ...
    function _safeChecks(address _baseToken,
        address _quoteToken,
        uint256 _amount,
        uint256 _price,
        bool _isBuy,
        uint8 _orderType) private view {
    ...
    function _native(address _baseToken, address _quoteToken, uint256 _amount, bool _isBuy) private returns (bool){
        if (_baseToken == address(0) && _quoteToken == address(weth)) {
            if (_isBuy) {
                if (!IERC20(_quoteToken).transferFrom(msg.sender, address(this), _amount)) {
                ...
                IERC20(_quoteToken).transfer(msg.sender, _amount);
                return true;
            }
        }
        return false;
    ...
    function _createOrder(
        ...
        uint256 holdAmount = _getHoldAmount(_baseToken, _quoteToken, _amount, _price, _isBuy);
        if (holdAmount == 0) {
            revert InvalidOrder();
        }
    ...
    */
        ...
        _safeChecks(_baseToken, _quoteToken, _amount, _price, _isBuy, _orderType);

        if (_native(_baseToken, _quoteToken, _amount, _isBuy)) {
            return 0;
        }

        uint256 newId = _createOrder(_baseToken, _quoteToken, _amount, _price, _isBuy, _orderType);
    ...
    */
    ...
    // @dev
    // edit order for new price/amount
    function editOrder(
        uint256 _orderId,
        uint256 _amount,
        uint256 _price
        ...
        if (order.amount == _amount && order.price == _price) {
            revert OrderNotChanged();
        }

        uint256 divider = 18 - IERC20Metadata(order.tokenA).decimals();
        uint256 oldHoldAmount = _getHoldAmount(
            order.tokenA, order.tokenB, order.amount, order.price, order.isBuy
        ) / 10**divider;

        uint256 newHoldAmount = _getHoldAmount(
            order.tokenA, order.tokenB, _amount, _price, order.isBuy
        );

        address holdToken = order.tokenA;
        if (order.isBuy) {
            holdToken = order.tokenB;
        }

        int256 diff = int256(newHoldAmount) - int256(oldHoldAmount);

        if (diff > 0) {
            if (!IERC20(holdToken).transferFrom(msg.sender, address(this), uint256(diff))) {
                revert TransferFailed();
            }
        }
        if (diff < 0) {
            if (!IERC20(holdToken).transfer(msg.sender, uint256(-diff))) {
                revert TransferFailed();
            }
        }
        orders[_orderId].price = _price;
        orders[_orderId].amount = _amount * 10**divider;
        orders[_orderId].initialAmount = newHoldAmount;
        orders[_orderId].spentAmount = 0;

        emit OrderEdited(msg.sender, _orderId, _amount, _price, order.tokenA, order.tokenB, block.timestamp);
    }
